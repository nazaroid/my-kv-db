...
class StatisticsIntegration[F[_]: Async: Logger](
  ...
  def getHealthCheck: F[HealthStatus] = {
    for {
      ...
      healthStatus =
        if (healthyDbs == totalDbs && avgFragmentation < 0.3) {
...
object HealthStatus {
  sealed trait Status derives Codec.AsObject
...
object StatisticsIntegration {
  ...
  def create[F[_]: Async: Logger: Files](
  ...
  def createWithAdapter[F[_]: Async: Logger: Files](
    storageManager: StorageManager[F],
