...
final class BitcaskTableOperationSpec extends AsyncFreeSpec with AsyncIOSpec with Matchers {
  ...
  "Recovery: non-ASCII key should survive restart" in {
    val key = "ключ_🔑"
    val value = "unicode value"

    val session1 = tableResource.use { t =>
      t.write(key, value) *> IO.sleep(100.millis)
    }

    val session2 = tableResource.use { t =>
      t.read(key).map { recoveredValue =>
        assert(recoveredValue.contains(value), "Non-ASCII key should be recovered from disk indexes")
      }
    }
    session1 *> session2
  }
