...
final class ServerModule[F[_]: Async: Files: Logger: Parallel: Network](commonModule: CommonModule[F]) {
  ...
  def resolve: Resource[F, Server[F]] = {
    for {
      engine <- BitcaskEngine.init[F](config.engine, commonModule.metricRegistry)
      _      <- Resource.eval(engine.startMonitoring())
    } yield {
      config.server match {
        case httpConf: ServerConfig.Http =>
          new org.nazaroid.kvdb.srv.http.HttpServer[F](httpConf, engine)
