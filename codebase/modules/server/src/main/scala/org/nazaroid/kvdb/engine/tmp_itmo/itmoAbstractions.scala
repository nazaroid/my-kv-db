package org.nazaroid.kvdb.engine.tmp_itmo
...
@FunctionalInterface trait DatabaseFactory {
  ...
  /** Создает базу данных с указанным именем, если такая база еще не существует.
    ...
    *   имя базы данных
    * @param dbRoot
    *   путь до директории, в которой будет создана база данных
    * @return
    *   объект созданной бд
    * @throws DatabaseException
    *   если база данных с данным именем уже существует или если произошла ошибка ввода-вывода
    */
  @throws[DatabaseException]
...
trait Database {
  ...
  /** Создает таблицу с указанным именем, если это имя еще не занято.
    ...
    *   имя таблицы
    * @throws DatabaseException
    *   если таблица с данным именем уже существует или если произошла ошибка ввода-вывода
    */
  @throws[DatabaseException]
  ...
  /** Записывает значение в указанную таблицу по переданному ключу.
    *
    * @param tableName
    *   таблица, в которую нужно записать значение
    * @param objectKey
    *   ключ, по которому нужно записать значение
    * @param objectValue
    *   значение, которое нужно записать
    * @throws DatabaseException
    *   если указанная таблица не была найдена или если произошла ошибка ввода-вывода
    */
  @throws[DatabaseException]
  def write(
    tableName: String,
    objectKey: String,
    objectValue: Array[Byte]
  ): Unit

  /** Считывает значение из указанной таблицы по заданному ключу.
    *
    * @param tableName
    *   таблица, из которой нужно считать значение
    * @param objectKey
    *   ключ, по которому нужно получить значение
    * @return
    *   значение, которое находится по ключу
    * @throws DatabaseException
    *   если не была найдена указанная таблица, или произошла ошибка ввода-вывода
    */
  @throws[DatabaseException]
...
/** Таблица - логическая сущность, представляющая собой набор файлов-сегментов, которые объединены одним именем и
  * используются для хранения однотипных данных (данных, представляющих собой одну и ту же сущность, например, таблица
  * "Пользователи") <p>
  *   - имеет единый размер сегмента
  *   - представляет из себя директорию в файловой системе, именованную как таблица и хранящую файлы-сегменты данной
  *     таблицы
  */
trait Table {
...
trait Segment {
  ...
  /** Записывает значение по указанному ключу в сегмент.
    ...
    *   значение, которое нужно записать
    * @return
    *   {@code true} - если значение записалось, {@code false} - если нет
    * @throws IOException
    *   если произошла ошибка ввода-вывода.
    */
  @throws[IOException]
  ...
  /** Возвращает {@code true} - если данный сегмент открыт только на чтение, {@code false} - если данный сегмент открыт
    * на чтение и запись.
    *
    * @return
    *   {@code true} - если данный сегмент открыт только на чтение, {@code false} - если данный сегмент открыт на чтение
    *   и запись
    */
...
trait KvsIndex[K, V] {
  ...
  /** Ищет значение в индексе по указанному ключу.
    *
    * @param key
    *   ключ, по которому нужно провести поиск значение
    * @return
    *   {@code Optional<V>}
    */
