...
final class HttpStatisticsSpec extends AsyncFreeSpec with AsyncIOSpec with Matchers {
  ...
  "GET /stats/catalog should return catalog statistics" in withDbServerRunning { client =>
    for {
      ...
      _ <- IO {
        val obj = response.asObject.get
        assert(obj("totalDatabases").exists(_.asNumber.exists(_.toInt.contains(1))), "Should have 1 database")
        assert(obj("totalTables").exists(_.asNumber.exists(_.toInt.contains(2))), "Should have 2 tables")
        assert(obj("totalEntries").exists(_.asNumber.exists(_.toInt.contains(3))), "Should have 3 entries")
        assert(obj("activeEntries").exists(_.asNumber.exists(_.toInt.contains(3))), "Should have 3 active entries")
  ...
  "Statistics should reflect data changes" in withDbServerRunning { client =>
    for {
      ...
      // Test deletion
      _ <- client.run(Request[IO](DELETE, Uri.unsafeFromString(f"$baseUrl/data/testdb/users/user2"))).use_

      // Get final stats
      finalRequest = Request[IO](Method.GET, Uri.unsafeFromString(f"$baseUrl/stats/table/testdb/users"))
  ...
  "Multiple databases statistics" in withDbServerRunning { client =>
    for {
      ...
      _ <- IO {
        ...
        assert(obj("totalTables").exists(_.asNumber.exists(_.toInt.contains(1))), "DB2 should have 1 table")
        assert(obj("totalEntries").exists(_.asNumber.exists(_.toInt.contains(1))), "DB2 should have 1 entry")
      }
