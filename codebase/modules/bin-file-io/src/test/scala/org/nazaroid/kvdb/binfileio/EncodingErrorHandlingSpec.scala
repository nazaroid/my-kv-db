...
final class EncodingErrorHandlingSpec extends AnyFunSuite with Matchers {
  ...
  test("decode should handle corrupted data gracefully") {
    ...
    val validEncoded = encode(validRow, schema).fold(err => fail(err), identity)
  ...
  test("decode should work without CRC validation when no CRC in schema") {
    ...
    val encoded = encode(row, schemaWithoutCRC).fold(err => fail(err), identity)
  ...
  test("encode should handle missing fields gracefully") {
    ...
    encode(rowWithoutRequired, schema) match {
      case Left(error) => error should include("Field valueSize missing")
      case Right(_)    => fail("Should not encode successfully with missing fields")
    }
  }
  ...
  test("encode should handle different field types correctly") {
    ...
    val encoded = encode(row, schema).fold(err => fail(err), identity)
  ...
  test("encode should reject UTF-8 size mismatch for StringUtf8 fields") {
    val schema = List(
    ...
    val row = Map(
      "keySize" -> 4, // character count, not UTF-8 byte length
      "key"     -> "ключ"
    )

    encode(row, schema) match {
      case Left(error) => error should include("declares 4 bytes but UTF-8 encoding is 8 bytes")
      case Right(_)    => fail("Should not encode successfully with UTF-8 size mismatch")
    }
  }

  test("encode and decode should round-trip non-ASCII UTF-8 strings") {
    val key = "ключ_🔑"
    val schema = List(
      FieldDef("keySize", FieldType.Int32),
      FieldDef("key", FieldType.StringUtf8(sizeFromField = "keySize"))
    ...
    val decoded = encode(row, schema).flatMap(chunk => decode(chunk, schema))
    decoded shouldBe Right(row)
  }
