...
final class MemStorageSpec extends AnyFlatSpecLike {
  import MemStorageSpec.*

  it should "write and read same data" in {
    (for {
      ix     <- Async[IO].ref(Map[String, MemStorageValue]())
      buffer <- Channel.bounded[IO, WriteTask[IO]](2)
      s = MemStorage[IO](ix, buffer, path, schema, keyField)
