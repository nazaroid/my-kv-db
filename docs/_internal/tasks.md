...
## gpt promt
```
Я разрабатываю kv-базу даных с движком bitcask на Scala 3 + Cats.
...
- (TODO) добавить метрики по скорости запись/чтение
  ...
  Я разрабатываю kv-базу даных с движком bitcask на Scala 3 + Cats.
  Чтобы разобраться в структуре проекта изучи документацию, начиная с README.md и документы в папке ./docs .
  Код kv-базы находится в ./codebase. А в папке ./playground тестовая среда для обкатки. 
  ...
  Добавь их поддержку для BitcaskEngine и выведи на дашборд Grafana в playground/monitoring/grafana/dashboards/kvdb-dashboard.json.
